...
function test(id){
  ...
  setTimeout(function() {
	...
	if(parentId == 0){
	  ...
	  stationId = idArry[0] ;
	  columnId = 0;
	  url = contextPath + "/cms/content/manageStation.jsp?stationId="+stationId+"&columnId="+columnId;
	}
	else{
	  var idArry = id.split(",");
	  columnId = idArry[0];
	  url = contextPath + "/cms/content/manage.jsp?stationId="+stationId+"&columnId="+columnId;
	}
...
function addContent(){
  if(columnId == 0){
    ...
    var node = tree.getNode(id);//node得到的是你点击的结点的一个js对象
    var extData = node.extData;//库seqId
    if(isPermissions(extData.editUserContent)){
      parent.file_main.location.href = contextPath + "/cms/content/new.jsp?stationId="+stationId+"&columnId="+columnId+"&treeId="+id;
    }
    else{
      alert("您没有该栏目新建文章内容的权限！");
    }
  }
}
...
<br>
<div align="center">
<input type="button"  value="新建文章" class="BigButtonC" onClick="addContent();" title="添加新的文章">
</div>
