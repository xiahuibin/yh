...
function doInit(){
  ...
  var requestURL = "<%=contextPath%>/yh/core/esb/server/taskstatus/act/YHTaskStatusAct/getDownloadPage.act";
  var cfgs = {
    ...
    colums: [
       ...
       {type:"text", name:"guid", text:"GUID", width:'12%',align:'center'},
       ...
       {type:"text", name:"createTime", text:"接收开始时间", width:'15%',align:'center' ,render:splitDateTime},
       {type:"text", name:"completetime", text:"接收结束时间", width:'15%',align:'center' ,render:splitDateTime},
       {type:"time", name:"time", text:"用时", width:'10%',align:'center'}
       ]
  };
...
function statusRender(cellData, recordIndex, columInde) {
  if (cellData * 1 >= 0 && cellData * 1 < 5) {
    return ["<span style=\"color:blue;\">等待接收</span>", "接收中", "<span style=\"color:green;\">接收完毕</span>", "","<a href=\"javascript:void(0)\" onclick=\"failedMessage("+recordIndex+")\" style=\"color:red;\" title=\"点击查看错误详情\">接收失败</a>"][cellData * 1];
  }
  else {
    return "";
  }
...
function ext(){
  ...
  location =  "<%=contextPath%>/yh/core/esb/server/taskstatus/act/YHTaskStatusAct/expDown.act?"+pars;
}
</script>
</head>
...
<table border="0" width="" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="<%=imgPath %>/system.gif" align="absmiddle"><span class="big3"> 下载任务状态</span><br>
