...
//输出列表信息
	var url=requestURL + "/getFileContentsById.act?seqId=<%=seqId%>&EXT_FILTER=<%=ext_filter%>&DIV_ID=<%=div_id %>&DIR_FIELD=<%=dir_field %>&NAME_FIELD=<%=name_field %>&TYPE_FIELD=<%=type_field %>&MULTI_SELECT=<%=multi_select %>";
