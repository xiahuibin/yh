function dealDis(selected,disselected){
   ...
   for(var i=0;i<selected.length;i++){
	...
	function getDisSelected(){
		 var rtJson = getJsonRs( contextPath + "/yh/core/funcs/workflow/act/YHFlowPrintAct/getDisSelectByFlowIdAct.act", "flow_id="+flow_id);
		  if (rtJson.rtState == "0") {
	...
	function getSelected(){
	     var rtJson = getJsonRs( contextPath + "/yh/core/funcs/workflow/act/YHFlowPrintAct/getSelectByFlowIdAct.act", "seq_id="+seq_id);
	      if (rtJson.rtState == "0") {
	        var data=rtJson.rtData;
	...
	function saveFile()
	{
	  ...
	  if(tName=="")
	  {
	    ...
	    var param="prcsStr="+prcsStr+"&seqId="+seq_id+"&tName="+ $("tName").value+"&flowId="+flow_id+"&tType="+$("tType").value+"&content="+content.replace(/\+/g, '%2B');
	     ...
	     var url =  contextPath + "/yh/core/funcs/workflow/act/YHFlowPrintAct/updateAipAct.act";
