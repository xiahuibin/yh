...
function deleteSms(seqId) {
  ...
  var url = "<%=contextPath %>/yh/core/funcs/message/act/YHMessageAct/deleteMessage.act";
...
function resetSms(smsBodyId,content,smsType,remindUrl){
  ...
  var rtJson = getJsonRs(url, "user="+toId+"&content="+encodeURIComponent(content)+"&messageType="+smsType+"&remindUrl="+encodeURIComponent(remindUrl));
  if (rtJson.rtState == "0") {
    alert("成功发送");
    window.location.reload();
  }
...
<table cellscpacing="1" cellpadding="3" width="100%">
  <tr>
    <td class="Big">
      <img src="<%=imgPath%>/msg_fwd.gif"></img><span class="big3"> 已发送微讯查询结果 ，最多显示200条结果</span>
    </td>
    <td id="numDiv" style="display:none;" align="center"></td>
      <td class="Big">
      <input type="button"  class="BigButton"value="返回" onclick="location='<%=contextPath %>/core/funcs/message/querymessage.jsp'"></input>
    </td>
...
<table cellscpacing="1" cellpadding="3" width="100%" id="sentSmscon" class="TableList">
  <tr class="TableHeader">
   ...
   for(int i = 0; i < data.getRecordCnt(); i++) {
     ...
     String smsBodyStr = record.getValueByName("messageBodyId").toString();
   ...
   <tr class="<%= (i % 2 == 0) ? "TableLine1" : "TableLine2" %>">
     ...
     <td>
       ...
       <a href="javascript:dispatchSms('<%=smsBodyId %>');">转发</a>&nbsp;&nbsp;
                 ...
                 if(fromId==loginId)
                 document.write(" <a href='javascript:resetSms(\"<%=smsBodyId%>\",\"\",\"<%=smsType %>\",\"<%=remindUrl %>\");'>重发</a>");
                 </script>
     </td>
   </tr> 
