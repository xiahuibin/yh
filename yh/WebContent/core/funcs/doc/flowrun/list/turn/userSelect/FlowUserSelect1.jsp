...
<%
  ...
  String parentFlowId = request.getParameter("parentFlowId");
  if (parentFlowId == null) {
    parentFlowId = "";
  }
  String backPrcs = request.getParameter("backPrcs");
  if (backPrcs == null) {
    backPrcs = "";
  }
  if (YHUtility.isNullorEmpty(userFilter)) {
    userFilter = "0";
  }
  String childFlow = request.getParameter("childFlow");
  if (YHUtility.isNullorEmpty(childFlow)) {
    childFlow = "0";
  }
...
<script type="text/Javascript" src="flowUserSelect1.js" ></script>
<script type="text/javascript">
var deptId = '<%=deptId%>';
var ctroltime=null,key="";
var userFilter = <%=userFilter%>;
var prcsChoose = '<%=prcsId%>';
var childFlow = '<%=childFlow%>';
var backPrcs = '<%=backPrcs%>';
var parentFlowId = '<%=parentFlowId%>';
...
function closeWindow(){
  parentWindowObj.$("selectUserTd" + prcsChoose).hide();
}
</script>
</head>
<body onload="doInit()">
...
<div style="position:absolute;top:335px;left:245px;width:60px;">
  <input type=button class="SmallButtonW" value="关闭" onclick="closeWindow()"/>
</div>
</body>
</html>