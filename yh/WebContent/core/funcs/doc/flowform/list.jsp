...
var sortId = '<%=sortId%>';
function doInit(){
  var url = contextPath + "<%=moduleSrcPath %>/act/YHFlowFormAct/listBySort.act";
  ...
  if(flowJson.length > 0){
	...
	for(var i = 0 ; i < flowJson.length ;i++){
	  var form = flowJson[i];
      if (form) {
  	  name = "<a href=\"javascript:openFormDesign(" + form.seqId + ");\">" + form.formName + "</a>";
  	  design = "<a href=\"javascript:openFormDesign(" + form.seqId + ");\">表单设计器</a>";
...
function deleteForm(seqId){
  ...
  var url = "<%=contextPath %><%=moduleSrcPath %>/act/YHFlowFormAct/deleteForm.act";
