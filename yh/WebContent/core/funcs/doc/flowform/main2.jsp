...
function send()
{
  if(CheckForm())
  {
   ...
   /// var url = "";
    ...
    if(seqId != null && seqId != ""){
      url = "<%=contextPath%><%=moduleSrcPath %>/act/YHFlowFormAct/updateForm.act";
    }else{
      url = "<%=contextPath%><%=moduleSrcPath %>/act/YHFlowFormAct/insertForm.act";
    }
...
function loadData(){
  var url = "<%=contextPath%><%=moduleSrcPath %>/act/YHFlowFormAct/getFormView.act?";
...
//--- 主题词选择控件 ---
function td_word_select() {
  var FCK = FCKeditorAPI.GetInstance('FORM_CONTENT');
  FCK.Focus();
  FCK.Commands.GetCommand("NWordSelect").Execute(); 
}
...
function createFck() {
  ...
  oFCKeditor.Config["CustomConfigurationsPath"] = "<%=moduleSrcPath %>/flowform/editor/fckconfig.js"; 
  ...
  oFCKeditor.Config["PluginsPath"] = "<%=moduleSrcPath %>/flowform/editor/plugins/";
