...
function addUser(i , userFilter , childFlow , parentFlowId , backPrcs){
  ...
  var chooseType = window.showModalDialog(contextPath + "/core/funcs/doc/flowrunRec/list/turn/userSelect/FlowUserSelect.jsp?prcsId=" + i + "&userFilter=" + userFilter + "&childFlow=" + childFlow + "&parentFlowId=" + parentFlowId+ "&backPrcs=" + backPrcs,window,"dialogLeft=480px;dialogTop=350px;dialogWidth=520px;dialogHeight=400px;status=no;resizable=no"); 
}
function setAction(){
  ...
  if (flag == 0) {
    ...
    flowView(runId , flowId ,title  , sortId , skin  , 1);
  } else {
	viewGraph(flowId);
  }
}
