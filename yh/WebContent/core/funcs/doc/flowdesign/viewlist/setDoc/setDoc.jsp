...
function doInit(){
  ...
  var docSmsStyle = json.rtData.docSmsStyle;
  $('docSmsStyle').value = docSmsStyle;
  var docSendFlag = json.rtData.docSendFlag;
  $('DOC_SEND_FLAG').value = docSendFlag;
}
function commit(){
...
<form  method="post" name="setFieldForm" id="setFieldForm">
  ...
  <tr class="TableContent">
    ...
    <td  class="TableContent" width=100px >是否发文：</td>
    <td  class="TableData" >
     <select id="DOC_SEND_FLAG" name="DOC_SEND_FLAG">
     <option value="" selected>否</option>
     <option value="1">是</option>
     </select>
    </td>
  ...
  <tr class="TableContent">
    <td  class="TableContent" width=100px >短信提醒模板：</td>
    <td  class="TableData"  colspan="3">
    <textarea rows="5" id="docSmsStyle" name="docSmsStyle" cols="30"></textarea>
    <div>说明：模板中可以使用以下特殊标记：<br>
    #{办理人}:代表所选择的下一步办理人<br>
    #{表单控件的名字}:代理对应表单控件的值<br>
    如：#{办理人}:#{标题}请你审核。拟稿人：#{拟稿人}<br>
    </div>
    </td>
  </tr>
  <tr>
