...
function doInit() {
  ...
  if (window.navigator.appVersion.match(/9./i)!='9.') {
    jso.push({title:"图形视图",useTextContent:true,imgUrl:imgPath + "/asset.gif", contentUrl:"<%=contextPath %>/core/funcs/doc/flowdesign/canvas.jsp?flowId=<%=flowId%>", useIframe:true});
  } else {
    jso.push( {title:"图形视图",useTextContent:true,imgUrl:imgPath + "/asset.gif", contentUrl:"<%=contextPath %>/yh/core/funcs/doc/act/YHFlowProcessAct/getProcessList1.act?flowId=<%=flowId%>", useIframe:true});
  }
  jso.push({title:"列表视图", useTextContent:true,imgUrl:imgPath + "/edit.gif", contentUrl:"<%=contextPath %>/core/funcs/doc/flowdesign/viewlist/prcslist.jsp?flowId=<%=flowId%>" , useIframe:true});
