...
function loadDataAction(obj){
  ...
  window.location.href = "<%=contextPath %>/yh/core/funcs/sms/act/YHSmsTestAct/sentSmsList.act?pageNo=" + pageNo + "&pageSize=" + pageSize ;
}
function showStatus(smsBodyId){
  var url = "<%=contextPath%>/yh/core/funcs/sms/act/YHSmsAct/getStatusByBodyId.act?bodyId="+smsBodyId;
  openWindow(url,"查看详情",900,600) ;
}
</script>
</head>
   ...
   <tr class="<%= (i % 2 == 0) ? "TableLine1" : "TableLine2" %>">
     ...
     <script>
       ...
       if(toId){
         ...
         if(toIds.length > 1 || toId == 0 || toId == "ALL_DEPT"){
