...
function goBack() {
  window.location.href = location = "<%=contextPath%>/core/funcs/person/usernew.jsp?deptId="+deptId;
}
...
function doInit() {
  jso = [
      {title:"用户基本信息", onload:showUserInfo.bind(window, "baseInfo"), useTextContent:true, contentUrl:"<%=contextPath %>/core/funcs/person/offdutypersonbasicinfo.jsp?userIdStr="+userIdStr+"&postPriv="+postPriv, useIframe:false}
     ,{title:"用户权限信息", onload:showUserInfo.bind(window, "privInfo"), useTextContent:true, contentUrl:"<%=contextPath %>/core/funcs/person/offdutypersonrangeinfo.jsp?userIdStr="+userIdStr, useIframe:false}
     ,{title:"其它选项", onload:showUserInfo.bind(window, "otherInfo"), useTextContent:true, contentUrl:"<%=contextPath %>/core/funcs/person/offdutypersonotherinfo.jsp", useIframe:false}
     ,{title:"用户可自定义选项", onload:showUserInfo.bind(window, "selfDef"), useTextContent:true, contentUrl:"<%=contextPath %>/core/funcs/person/offdutypersondifineinfo.jsp", useIframe:false}
     ]; 
...
%>
    <span class="big1"><img src="<%=contextPath %>/core/styles/imgs/edit.gif"></img>&nbsp;用户编辑</span>
<%
  }
%> 
