...
function doInit(){
    var url = contextPath + "/yh/core/funcs/connswitch/act/YHConnSwitchAct/getSqlConnInfo.act";
...
function testDbConnect(){
    var url = contextPath + "/yh/core/funcs/connswitch/act/YHConnSwitchAct/testSqlConnect.act";
    var param="";
...
function saveDbConnect(){
	if(window.confirm("确认是否要保存Sqlserver配置信息！")){
    var url = contextPath + "/yh/core/funcs/connswitch/act/YHConnSwitchAct/saveSqlConnect.act";
    var param="";
    param="hostName="+$("hostName").value+"&socket="+$("socket").value+"&dbase="+$("dbase").value+"&uid="+$("uid").value+"&pwd="+$("pwd").value;
     var rtJson = getJsonRs(url,param);
      if(rtJson.rtState == "0"){ 
           alert("Sqlserver数据库配置成功保存！");
      }else{
        alert( rtJson.rtMsrg);
      }
    }
...
<table border="0" width="90%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td><img src="<%=imgPath%>/notify_new.gif" align="absmiddle"><span class="big3"> Sqlserver连接配置信息</span>&nbsp;&nbsp;
    </td>
  </tr>
</table>
...
<table class="TableBlock" width="70%" align="center">
  ...
  <tr>
  <td colspan="2" class="TableContent" align="right"><input type="button" onclick="saveDbConnect()" class="BigButton" value="保 存"></td>  
