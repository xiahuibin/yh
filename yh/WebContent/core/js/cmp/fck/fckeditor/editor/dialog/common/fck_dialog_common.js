...
function getFunContext()
{
  var images = fckdocument.getElementsByTagName("img");
  for(var i = 0 ;i < images.length ; i++){
    var image = images[i];
    if(image.getAttribute("widgettype") == 'Function'){
      var scriptText =  "function " + image.id + "(){return '"+ image.id +"';}";
...
function getIndexOfPreName(pre,fckdocument){
  var max = 0;
  if (dialog && dialog.parent.isSave2DataTable) {
    dialog.parent.itemMax++;
    return dialog.parent.itemMax;
  } else {
    if(pre && pre == "DATA"){
      var inputDoms = fckdocument.getElementsByTagName("input");
      var imgDoms = fckdocument.getElementsByTagName("img");
      var textareaDoms = fckdocument.getElementsByTagName("textarea");
      var selectsDoms = fckdocument.getElementsByTagName("select");
      var buttonDoms = fckdocument.getElementsByTagName("button");
      for(var i = 0 ;i < inputDoms.length ; i++){
        var input = inputDoms[i];
        if(input.name){
          var tem = (input.name).replace("DATA_","");
          max = isNaN(Math.max(max,parseInt(tem))) ? max :Math.max(max,parseInt(tem)) ;
        }
      }
      for(var i = 0 ;i < imgDoms.length ; i++){
        var img = imgDoms[i];
        if(!img.className || img.className == "USER" || img.className == "DATE"){
          continue;
        }
        if(img.name){
          var tem = (img.name).replace("DATA_","");
          max = isNaN(Math.max(max,parseInt(tem))) ? max :Math.max(max,parseInt(tem)) ;
        }
      }
      ...
      for(var i = 0 ;i < buttonDoms.length ; i++){
        var button = buttonDoms[i];
        if(button.name){
          var tem = (button.name).replace("DATA_","");
          max = isNaN(Math.max(max,parseInt(tem))) ? max :Math.max(max,parseInt(tem)) ;
        }
      }
...
function checkUnique(title,fckdocument){
  ...
  var doms = new Array();
  if(fckdocument.all){
    doms = fckdocument.all;
   // fckdocument.all = Array.prototype.slice.apply(this, fckdocument.all);
  }else{
    var inputDoms = fckdocument.getElementsByTagName("input");
    var textareaDoms = fckdocument.getElementsByTagName("textarea");
    var selectsDoms = fckdocument.getElementsByTagName("select");
    var buttonDoms = fckdocument.getElementsByTagName("button");
    doms = doms.concat(inputDoms,textareaDoms,selectsDoms,buttonDoms);
  }
  //doms不是一个标准的doms
  //doms = Array.prototype.slice.apply(this, doms);
  //alert(doms);
...
function CreateNamedElement( oEditor, oOriginal, nodeName, oAttributes )
{
	...
	else
	{
        ...
        if ( !oEditor.FCKListsLib.EmptyElements[nodeName.toLowerCase()] )
          sbHTML.push( '</' + nodeName + '>' ) ;
        oNewNode = oEditor.FCK.EditorDocument.createElement( nodeName ) ;
        for( var prop in oAttributes )
        {
          oNewNode.setAttribute( prop , oAttributes[prop] ) ;
        }
		  }
	...
	for( var attName in oAttributes ){
	  ...
	  try{
	    ...
	    oNewNode[attName] = oAttributes[attName];
	  }
	}// 0 : Case Insensitive
  //alert(oNewNode);
	return oNewNode ;
