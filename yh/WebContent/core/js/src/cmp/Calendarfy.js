/**
 * 1.修复一些bug.加入了秒。以及改变了选择时间的方式 ---update by lh 2010-3-1
 * 2.只允许一个div弹出。---update by lh 2010-3-4
 */
var Calendar = Class.create();
Calendar.prototype = {
   /*{bindToBtn,inputId,property:{week,yearRange,month,format,ishaveTime}}
    * bindToBtn 字符串 绑定的按钮id,不指定日历会默认显示在输入框下面
    * inputId 字符串 日历输入框的id
    * property 对象 主要设置日历的一些属性



    *   week 字符串数组 指定日历显示星期的格式



    *   month 字符串数组 指定日历显示月份的格式



    *   yearRange 整型数组 指定年的显示范围
    *   format 字符串 日期的输出格式



    *   ishaveTime boolean 是否显示时间 
    */
  initialize: function(parameters) {
    ...
    if(parameters.property && parameters.property.yearRange){
      ...
      this.yearRange = [1920,2030];//数组第一个为开始年份，第二个为结束年份
    }
  ...
  createCalDiv:function(e){
   ...
   // this.div.focus();
    ...
    var temp="<div id='" + this.divId +"Top' class='top'><b class='arrow'><div class='left'></div></b><b class='arrow' style='float:right;'><div class='right'></div></b></div>"
      ...
      + "<div class='bottom'><div class='btn'>今日</div><div id='" + this.divId + "Time' class='time'></div><div class='btn' style='float:right'>取消</div></div>";
    ...
    if(this.isHaveTime){
      ...
      this.inputHour = new Element('input',{id:this.divId + 'HourInput'});
      //分
      ...
      this.selectMM.hide();
      this.inputMM = new Element('input',{id:this.divId + 'MMInput'});
      //秒


      this.selectSecond = new Element('select' , {id : this.divId + 'Second'}).update(temp);
      this.selectSecond.value = this.date.second;
      this.selectSecond.hide();
      this.inputSecond = new Element('input',{id:this.divId + 'SecondInput'});
      ...
      this.setTime(this.selectHour.value, this.selectMM.value, this.selectSecond.value);
    }
  ...
  initDatesByYM:function(year,month){
    ...
    if(first>0)
      var temp="<div style='width:"+(27*first)+"px;height:18px;float:left;'></div>";//占位而已
    else
      var temp="";
    var i;
    now = new Date();
  ...
  registerListener:function(){
    ...
    if(this.isHaveTime){
      this.inputHour.observe('mousedown',this.inputHourFouceHandler.bindAsEventListener(this));
      this.selectHour.observe('blur' , this.hourChangeHandler.bindAsEventListener(this));
      
      this.inputMM.observe('mousedown',this.inputMMFouceHandler.bindAsEventListener(this));
      this.selectMM.observe('blur' , this.minuteChangeHandler.bindAsEventListener(this));
      
      this.inputSecond.observe('mousedown',this.inputSecondFouceHandler.bindAsEventListener(this));
      this.selectSecond.observe('blur' , this.secondChangeHandler.bindAsEventListener(this));
    ...
    //this.inputMM.observe('blur',this.selectYearBlurHandler.bindAsEventListener(this));
    ...
    //alert(this.div.lastChild.firstChild);
