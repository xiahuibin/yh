...
<frameset rows="*"  cols="180,*" frameborder="NO" border="0" framespacing="0" id="frame1">
    <frame name="file_tree" id="file_tree" scrolling="auto" src="left.jsp?flowId=<%=flowId %>&paperTimes=<%=paperTimes %>&userId=<%=userId %>&paperId=<%=paperId %>"  frameborder="no">
    <frame name="hrms" id="hrms" scrolling="auto" src="<%=contextPath %>/subsys/oa/examManage/examOnline/queryExam/quizList.jsp?flowId=<%=flowId %>&paperTimes=<%=paperTimes %>&currPage=1&userId=<%=userId %>&paperId=<%=paperId %>" frameborder="no">
</frameset>