...
function doInit(){	
  ...
  // }
   ...
   table =  table + "<tr align='center'><td><input type='button' class='SmallInput' value='   保存        ' onclick='funcExit("+currentPageIndex+",0);'></td></tr>";
   
   table =  table + "<tr align='center'><td><input type='button' class='SmallInput' value='   提交       ' onclick='funcExit("+currentPageIndex+",1);'></td></tr>";
...
function addData(){
  ...
  var url= "<%=contextPath%>/yh/subsys/oa/examManage/act/YHExamDataAct/addData2.act?flowId=" + flowId + "&userId="+userId + "&groupId="+groupId;
  var rtJson = getJsonRs(url); 
  if(rtJson.rtState == "1"){
	alert(rtJson.rtMsrg); 
	return ; 
...
function turnPage(currPage){
  ...
  parent.hrms.doInit();
  parent.hrms.location="<%=contextPath %>/subsys/oa/hr/score/month/scoreList.jsp?flowId=<%=flowId %>&userId=<%=userId %>&currPage="+currPage+"&groupId="+groupId;
 
 document.location.href ="<%=contextPath %>/subsys/oa/hr/score/month/left.jsp?flowId=<%=flowId %>&userId=<%=userId %>&currPage="+currPage+"&groupId="+groupId;
 //document.form1.submit();
}
function funcExit(curpage, checkEnd){
  //parent.hrms.form1.NEXT_PAGE.value=curpage;
  //msg='';
 //if(window.confirm(msg)){
  //URL="index.php";
    parent.hrms.doInit(checkEnd);
