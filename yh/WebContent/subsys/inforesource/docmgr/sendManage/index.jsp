...
function loadFileName() {
  var url = contextPath + "/yh/subsys/inforesouce/docmgr/act/YHDocAct/getDocType.act";
...
function loadWord(type) {
  ...
  var url = contextPath + "/yh/subsys/inforesouce/docmgr/act/YHDocAct/getDocWordByType.act";
...
function createNewDoc(){
  ...
  if (!$('flowId').value || !$('docType').value ) {
    alert("请为文档类型:"+ getName($('docType').value , fileName)+"绑定相应的流程！");
    return ;
  }
  var flowId = $('flowId').value;
  var word = getName($('docWord').value , docWords);
  var style= getStyle($('docWord').value);
  var url = contextPath + "/yh/subsys/inforesouce/docmgr/act/YHDocAct/createWorkFlow.act";
  var json = getJsonRs(url , "flowId=" + flowId + "&skin=" + skin + "&word=" + word + "&docStyle=" + style + "&docType=" + $('docType').value + "&wordId=" + $('docWord').value);
  if(json.rtState == "0"){
    location = contextPath + "/core/funcs/workflow/flowrun/list/inputform/index.jsp?skin=<%=skin%>&sortId="+ sortId +"&runId=" + json.rtData.runId + "&flowId=" + flowId + "&prcsId=1&flowPrcs=1&isNew=1";
  } else {
    alert(json.rtMsrg);
  }
}
