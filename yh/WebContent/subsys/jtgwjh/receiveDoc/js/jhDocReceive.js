...
function toDocTitle(cellData, recordIndex, columInde){
  var seqId = this.getCellData(recordIndex,"seqId");
  return "<a href='javascript: selectInfo("+seqId+ ")' >" + cellData+  "</a>";
}

function toDocTitleInfo(cellData, recordIndex, columInde){
  var seqId = this.getCellData(recordIndex,"seqId");
  return "<a href='javascript: selectInfo2("+seqId+ ")' >" + cellData+  "</a>";
}
 ...
 * @return
 */
function selectInfo2(seqId){
  var url = contextPath + "/subsys/jtgwjh/receiveDoc/manage/detailsIndex.jsp?seqId=" + seqId;
 //window.location.href = url;
  newWindow(url,800,550,"info");
}
