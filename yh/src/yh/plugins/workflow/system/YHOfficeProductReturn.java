...
public class YHOfficeProductReturn implements YHIWFHookPlugin {
  ...
  public String execute(Connection conn, int runId, Map arrayHandler,
      Map formData, boolean agree) throws Exception {
        ...
        String TRANS_QTY=(String)arrayHandler.get("TRANS_QTY");
        ...
        if(agree)
        {
              String update="update oa_office_trans_records set TRANS_QTY='"+TRANS_QTY + "',OPERATOR='"+BORROWER_ID+"',TRANS_STATE='1',PRICE='"+PRO_PRICE+"' where SEQ_ID='"+TRANS_ID+"'";
              YHWorkFlowUtility.updateTableBySql(update, conn);
              int NEW_PRO_STOCK=Integer.parseInt(PRO_STOCK)+Integer.parseInt(TRANS_QTY);
              update="update oa_office_goods set PRO_STOCK ='"+NEW_PRO_STOCK+"' where SEQ_ID='"+PRO_ID+"'";
              YHWorkFlowUtility.updateTableBySql(update, conn);
        }
        else
        {
