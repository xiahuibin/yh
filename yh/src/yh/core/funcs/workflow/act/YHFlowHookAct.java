...
import yh.core.funcs.workflow.logic.YHFlowHookLogic;
  ...
  public String updateHook(HttpServletRequest request,
    ...
    try{
      ...
      if (!YHUtility.isNullorEmpty(hid)) {
        ...
        String query = "update oa_fl_hook set hname='"+ hname +"',hdesc='"+ hdesc +"',status='"+status+"'";
        ...
        if ("1".equals(openedplugin)) {
          ...
          conditionSet = conditionSet.replace("AND[", "AND [");
          conditionSet = conditionSet.replace("OR[", "OR [");
          query += ",condition_set='"+conditionSet+"',oa_fl_hook.condition='"+condition+"'";
          if ("0".equals(system)) {
