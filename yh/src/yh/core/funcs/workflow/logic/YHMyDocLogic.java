...
public class YHMyDocLogic {
  public String getMyWork(Connection conn , YHPerson user , int maxCount  , String sortId) throws Exception {
    ...
    String query = "SELECT FLOW_RUN_PRCS.PRCS_ID " 
      ...
      + " , PERSON.USER_NAME"
      + " from oa_fl_run_prcs as FLOW_RUN_PRCS,oa_fl_run as FLOW_RUN,oa_fl_type as FLOW_TYPE, PERSON WHERE  " 
      ...
      while (rs.next()) {
        ...
        if ("1".equals(flowType)) {
          if (!prcsNameMap.containsKey(sFlowPrcsI)) {
            String query3 = "select PRCS_NAME from oa_fl_process WHERE FLOW_SEQ_ID="+ flowId +" AND PRCS_ID=" + flowPrcs;
            Statement stm3 = null;
            ResultSet rs3 = null;
            try {
        ...
        YHFlowRunData rd6 =  wf.getFlowRunData(conn, flowId, runId, "标题");
        String title = "";
        if (rd6 != null) {
          title = YHUtility.encodeSpecial(YHUtility.null2Empty(rd6.getItemData()));
        }
        sb.append("{");
  ...
  public String getFocusWork(Connection conn, YHPerson user, int maxCount , String sortId) throws Exception {
    ...
    String query = "SELECT FLOW_RUN.RUN_ID " 
      ...
      + " ,FLOW_RUN.FOCUS_USER "
      + " ,PERSON.USER_NAME"
      + "  FROM oa_fl_run as FLOW_RUN ,oa_fl_type as FLOW_TYPE,PERSON  WHERE  " 
