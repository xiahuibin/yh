...
public class YHFlowFormLogicUtil {
  private static Logger log = Logger.getLogger(YHFlowFormLogicUtil.class);
  int seqID = 0;
  ...
  public void updateJs(Connection conn, int seqId, String script)throws Exception {
    ...
    try{   
      String sql = "UPDATE oa_fl_form_type SET SCRIPT = ? where SEQ_ID =" + seqId;
  ...
  public void updateCss(Connection conn, int seqId, String css)throws Exception {
    PreparedStatement pstmt = null;
    try{   
      String sql = "UPDATE oa_fl_form_type SET CSS = ? where SEQ_ID =" + seqId;
      pstmt = conn.prepareStatement(sql);
  ...
  public String getPrcsList(Connection dbConn, int seqId) throws Exception {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String sql = "SELECT PRCS_USER, SEQ_ID FROM oa_fl_process WHERE SEQ_ID = " + seqId;
    try {
      stmt = dbConn.createStatement();
      rs = stmt.executeQuery(sql);
      while (rs.next()) {
