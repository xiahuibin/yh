...
public class YHFlowHookUtility {
  ...
  public String runHook(Connection conn, YHPerson user  ,Map dataMap,  String module ) throws Exception {
    String query = "select * from oa_fl_hook where hmodule='"+ module +"' and status >0";
    Statement stm = null; 
    ResultSet rs = null; 
    ...
    try { 
      ...
      if (rs.next()){ 
        ...
        String query1 = "insert into oa_fl_run_hook (run_id,module,field,key_id) values('"+runId+"','"+module+"','"+field+"','"+keyId+"')";
        YHWorkFlowUtility.updateTableBySql(query1, conn);
        return "/core/funcs/workflow/flowrun/list/turn/turnnext.jsp?runId=" + runId + "&flowId=" + flowId + "&prcsId=1&flowPrcs=1";
      } 
    } catch(Exception ex) { 
      throw ex; 
    } finally { 
 ...
 * @throws Exception
 */
public String runERPHook(Connection conn, YHPerson user  ,Map dataMap,  String module ) throws Exception {
	    String query = "select * from oa_fl_hook where hmodule='"+ module +"' and status >0";
	    Statement stm = null; 
	    ResultSet rs = null; 
	    Map arrayData = new HashMap();
	    ...
	    try { 
	      ...
	      if (rs.next()){ 
	        ...
	        String query1 = "insert into oa_fl_run_hook (run_id,module,field,key_id) values('"+runId+"','"+module+"','"+field+"','"+keyId+"')";
	        YHWorkFlowUtility.updateTableBySql(query1, conn);
	        return "/core/funcs/workflow/flowrun/list/inputform/index.jsp?skin=&sortId=&runId=" + runId + "&flowId=" + flowId + "&prcsId=1&flowPrcs=1";
	      } 
	    } catch(Exception ex) { 
	      throw ex; 
	    } finally { 
  ...
  public String runHookPlugin(Connection conn , int runId) throws Exception {
    String query = "SELECT * from oa_fl_hook a left outer join oa_fl_run_hook b on a.hmodule = b.module where b.run_id='"+runId+"'";
    Statement stm = null; 
    ResultSet rs = null; 
    Map arrayData = new HashMap();
    try { 
      ...
      if (rs.next()){ 
        ...
        String notPass = "";
        String query2 = "SELECT FORM_SEQ_ID , FLOW_TYPE.SEQ_ID from oa_fl_type as FLOW_TYPE , oa_fl_run as FLOW_RUN WHERE RUN_ID=" + runId + " AND FLOW_RUN.FLOW_ID = FLOW_TYPE.SEQ_ID";
        int formId = 0;
        int flowId = 0;
        ...
        Map formData = turnUtility.getForm(formId, runId , flowId, conn);
        ...
        ResultSet rs3 = null; 
        String map = "";
        String query3 = "select map from oa_fl_hook where SEQ_ID='"+seqId+"'";
        try { 
  ...
  public int getFlowId(Connection conn , String field ,String keyId) throws Exception{
    String query  = "select run_id from oa_fl_run_hook where field='"+ field + "' and key_id='"+keyId+"'";
    Statement stm = null; 
    ResultSet rs = null; 
    int runId = 0;
    try { 
  ...
  /**
   * 根据业务keyId，业务类别(就是FIELD)，取出工作流流水号
   * @param conn 
   * @param field 业务类别
   * @param keyId 业务id
   * @return 流水号
  ...
  public void deleteHook(Connection dbConn , String field , String keyId) throws Exception {
    ...
    if (runId != 0) {
      String delete = "delete from oa_fl_run_hook where  field='"+field+"' and key_id='"+keyId+"'";
