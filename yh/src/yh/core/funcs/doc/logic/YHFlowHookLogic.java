...
import yh.core.funcs.doc.data.YHDocFlowHook;
...
public class YHFlowHookLogic {
  public String configPath = YHWorkFlowConst.MODULE_CONTEXT_PATH + "/workflowUtility/flow_hook_config.properties";
  public String getHook(Connection conn,int hid , String webrootPath)  throws Exception{
    String query = "select * from "+ YHWorkFlowConst.FLOW_HOOK +" where SEQ_ID =  " + hid;
    Statement stm = null;
    ResultSet rs = null;
    ...
    String query2 = "SELECT FLOW_TYPE.SEQ_ID,FLOW_NAME,FLOW_TYPE from "+ YHWorkFlowConst.FLOW_TYPE +" as FLOW_TYPE,"+ YHWorkFlowConst.FLOW_SORT +" as FLOW_SORT where FLOW_TYPE.FLOW_SORT=FLOW_SORT.SEQ_ID order by SORT_NO,FLOW_NO";
    Statement stm2 = null;
    ResultSet rs2 = null;
    sb.append(",flows:[");
    int count = 0 ;
    ...
    String query3 = "select FORM_SEQ_ID FROM "+ YHWorkFlowConst.FLOW_TYPE +" WHERE SEQ_ID =" + flowId;
  ...
  public void addHookLogic(Connection conn,Map request,YHPerson person)  throws Exception{
    ...
    try{
      YHDocFlowHook hook=new YHDocFlowHook();
      hook.setSystem("0");
  ...
  public String getHookJsonLogic(Connection dbConn, Map request, YHPerson person ,String webrootPath ) throws Exception {
    try {
      String sql = " select c1.seq_id,c1.hmodule, c1.hname, c1.hdesc, c1.flow_id, c1.map, c1.plugin,c1.status,c1.system,1 "
                 + " from "+ YHWorkFlowConst.FLOW_HOOK +" c1 "
  ...
  public String getFlowNameLogic(Connection dbConn,String flowId) throws Exception {
   ...
   String flowName="";
    try {
      String sql ="select flow_name from "+ YHWorkFlowConst.FLOW_TYPE +" where seq_id='"+flowId+"'";
  ...
  public void deleteHookLogic(Connection dbConn,String seqId) throws Exception {
    Statement stmt=null;
     try {
        String sql="delete from "+ YHWorkFlowConst.FLOW_HOOK +" where seq_id='"+seqId+"'";
        stmt=dbConn.createStatement();
