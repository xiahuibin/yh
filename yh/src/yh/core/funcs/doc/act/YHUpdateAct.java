  ...
  public String importDelFlag(HttpServletRequest request,
    ...
    try{
      YHRequestDbConn requestDbConn = (YHRequestDbConn) request
          .getAttribute(YHBeanKeys.REQUEST_DB_CONN_MGR);
      dbConn = requestDbConn.getSysDbConn();
      String query = "select RUN_ID , DEL_FLAG from "+ YHWorkFlowConst.FLOW_RUN;
      Statement stm = null;
      ResultSet rs = null;
      try {
        stm = dbConn.createStatement();
        ...
        while (rs.next()) {
          ...
          String update = "update "+ YHWorkFlowConst.FLOW_RUN_PRCS +" set DEL_FLAG=" + delFlag + " where RUN_ID = " + runId;
          Statement stm2 = null;
          try {
...
//  }
  ...
  public String test() {
    ...
    String query = "SELECT "
      ...
      + " , c.BEGIN_USER "
      + " , c.END_TIME  "
      + "  from "+ YHWorkFlowConst.FLOW_RUN_PRCS +" AS a, "+ YHWorkFlowConst.FLOW_TYPE +" AS b, "+ YHWorkFlowConst.FLOW_RUN +" AS c  "
      + " WHERE  "
